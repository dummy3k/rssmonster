...
<a href="${h.url_for(controller='feed', action='update', id=feed.id, return_to=h.url_for())}"/>Update</a>
<a href="${h.url_for(controller='bayes', action='show_guesser', id=feed.id)}"/>Guesser</a>
<a href="${h.url_for(controller='bayes', action='redo', id=feed.id, return_to=h.url_for())}"/>Redo</a>
</%def>
